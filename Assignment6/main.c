...
while(1){
	disp(i);
       X=(i>>2)&one;Y=(i>>1)&one;Z=(i>>0)&one;	
       out=(X&Y)|(Y&Z);//display number
       dbg_hex8(X);dbg_str("\t\t");dbg_hex8(Y);dbg_str("\t\t");dbg_hex8(Z);dbg_str("\t\t");dbg_hex8(out);dbg_str("\n\n");
       PyHal_GPIO_Set(18,out&one);
